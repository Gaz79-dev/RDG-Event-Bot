...
@router.get("/{event_id}/tentative-signups", response_model=List[Signup])
async def get_tentative_signups(event_id: int, db: Database = Depends(get_db)):
    """Gets the roster of tentative signups for an event."""
    if not BOT_TOKEN or not GUILD_ID:
        raise HTTPException(status_code=500, detail="Bot token or Guild ID not configured on server.")
    
    tentative_records = await db.get_tentative_signups_for_event(event_id)
    roster, headers = [], {"Authorization": f"Bot {BOT_TOKEN}"}
...
@router.post("/{event_id}/accept-tentative", status_code=204, dependencies=[Depends(check_event_lock)])
async def accept_tentative_player(event_id: int, request: AcceptTentativeRequest, db: Database = Depends(get_db)):
    """Updates a player's RSVP from Tentative to Accepted and assigns their role."""
    await db.accept_tentative_player(event_id, request.user_id, request.role_name, request.subclass_name)
