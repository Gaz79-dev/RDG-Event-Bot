...
async def create_event_embed(bot: commands.Bot, event_id: int, db: Database) -> discord.Embed:
    ...
    if creator_id := event.get('creator_id'):
        try:
            if creator := (guild.get_member(creator_id) or await guild.fetch_member(creator_id)):
        ...
        if self.end_time.value:
            try:
                if start_dt := data.get('start_time'):
                    end_dt = start_dt.replace(hour=datetime.datetime.strptime(self.end_time.value, "%H:%M").time().hour, minute=datetime.datetime.strptime(self.end_time.value, "%H:%M").time().minute, second=0, microsecond=0)
...
class EventManagement(commands.Cog):
    ...
    async def create(self, interaction: discord.Interaction, channel: discord.TextChannel):
        await interaction.response.send_modal(EventCreationModal(self.bot, self.db, channel))
